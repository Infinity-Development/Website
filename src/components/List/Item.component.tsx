...
interface ItemProps extends WithChildren {
	visibility: boolean;
	state?: 'active' | 'maintenance' | 'archived';
				...
				<div className="flex flex-1 items-center justify-start w-full">
					...
					<div className="min-w-0 flex-1 px-4">
						<h1 className="text-gray-700 dark:text-white text-lg font-bold">
							{title} {" "}
							{state == 'archived' && (
								<Pill.Standard className="text-xs mt-4 w-auto h-7">{state === 'archived' && 'Archived/Deprecated'}</Pill.Standard>
							)}
							{visibility && (
								< Pill.Error className="text-xs mt-4 w-auto h-7">{visibility && 'Private Repo'}</Pill.Error>
							)}
						</h1>
						{description && (
